...
public class Punctuation
{
	...
	public String getPunctuationWords(String textIn)
	{
		...
		while (indexOfText < textIn.length()) //run through the text 
		{
			...
			{
				//in another method, go back from the index of text until
					//the char = space. then get the index of the space, and 
					//substring the text from the start of the word to the punctuation. 
						//(NOTE: special case: "blah blah
					//
			}
			indexOfText++;
		}
	}
	public boolean checkForPunctuation(char charAtIndex)
	{
		charStringAtIndex = "" + charAtIndex;
		if (charStringAtIndex == "." || charStringAtIndex == "," || 
			charStringAtIndex == ";" || charStringAtIndex == ":" || 
			charStringAtIndex == "!" || charStringAtIndex == "?" || 
			charStringAtIndex == "\'" || charStringAtIndex == "\"" || 
			charStringAtIndex == "..." || charStringAtIndex == "(" || 
			charStringAtIndex == ")" || charStringAtIndex == '-')
		{
			//go back from this index to space, then substring the word 
