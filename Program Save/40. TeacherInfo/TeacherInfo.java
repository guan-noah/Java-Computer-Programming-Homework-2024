...
class TeacherInfo
{
	...
	public void fakeMain()
	{
		...
		{
            String errorMessage = "Since " + teacherData[0] + " does not teach " + 
                "course number " + userCourseNumber + ", there is no data to report.";
            System.out.println(errorMessage);
            fileOutput.println(errorMessage);
		}
        fileOutput.close();
	...
	**/
	...
	public void outputData(int numOfCoursesIn)
	{
        ...
        ///PRINT TEACHER AND CLASS INFO (both)
        ...
        fileOutput.println("Data for: " + teacherData[0]);
        fileOutput.print("Course number: " + teacherData[2] + "\t");
        fileOutput.println("Course: " + teacherData[1]);
        fileOutput.println("Number of sections: " + numOfCoursesIn);
        fileOutput.println("Total # of scores = " + totalNumOfScores + "\n");
        
        System.out.println("\nData version 1: All scores printed from high to " + 
            "low: (terminal window only)");
        int numPrinted = 0;
        ...
        fileOutput.println("\nData version 2: Here are all of the scores grouped " + 
            "together by 90's, 80's etc. (text file only)");
        numPrinted = 0;
        for(int i = scores.length - 1; i >= 0; i--)                             //i = index in array
        {
            for(int scoreRepetition = 0; scoreRepetition < scores[i]; scoreRepetition++)
            {
                fileOutput.printf("%5d", i);
                numPrinted++;
                if(numPrinted%15 == 0)
                {
                    fileOutput.println(/*"\tnumPrinted = " + numPrinted*/);
                    numPrinted = 0;
                }
            }
            if(i%10 == 0 && i != 100)
        ...
        String[] letterGrade = new String[] {"A (90 - 100)", "B (80 - 89)", 
            "C (70 - 79)", "D (60 - 69)", "F (0 - 59)"};
        System.out.println("\nData version 3: Total number of each letter grade " + 
            "(both terminal and text file)");
        fileOutput.println("\nData version 3: Total number of each letter grade " + 
            "(both terminal and text file)");
        for(int i = 0; i < grades.length; i++)
        {
            System.out.printf("%-12s:%,4d%8.2f%%\n", letterGrade[i], grades[i], ((double)grades[i]*100)/totalNumOfScores);
            fileOutput.printf("%-12s:%,4d%8.2f%%\n", letterGrade[i], grades[i], ((double)grades[i]*100)/totalNumOfScores);
        }
