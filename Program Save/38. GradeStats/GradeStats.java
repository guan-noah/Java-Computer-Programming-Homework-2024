...
class GradeStats
{
    ...
    public void calcResults(int[] scoresIn)                                     //
    {
        ...
        for(int i: scoresIn)
            System.out.print(i + "\t");
        */
        int[] unsorted = new int[scoresIn.length];	//d&i unsorted to new array with scoresIn info
        for(int i = 0; i < scoresIn.length; i++)
        {
			unsorted[i] = scoresIn[i];
		}
        ...
        int[] under75 = getUnder75(unsorted);
    ...
    //returns the indices of students under 75 
