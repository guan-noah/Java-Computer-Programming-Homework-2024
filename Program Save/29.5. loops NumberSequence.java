...
How do you go from one number to the next?	add 2
Correct! Congratulations! The pattern was add 2. 
...
2. Input: 0; 10; increMent by 2; nO
...
How do you go from one number to the next?	increMent by 2
Correct! Congratulations! The pattern was increMent by 2. 
Would you like to play again? [Yes/No] 	nO
Alright. Thank you for playing NumberSequence! 
...
3. SPECIAL CASE - Input: 25; 35; 30; iNcREMent by 6; no
    //this showcases the bounds of the "close" part of the code (if user answer is within the sequence increment of the actual answer)
...
Close! 
What's the next number?	35
Close! 
What's the next number?	30
Correct! Well done. 
Whew. You got the answer correct on the last try. 
How do you go from one number to the next?	iNcREMent by 6
Correct! Congratulations! The pattern was iNcREMent by 6. 
Would you like to play again? [Yes/No] 	no
Alright. Thank you for playing NumberSequence! 
...
4. SPECIAL CASE - Input: 46; 64; 55; increase bY 9; yes; 0; 0; 0; i don't know; no
    //special case as I run this more than once and show bounds of "close"
...
Not quite! 
What's the next number?	55
Correct! Well done. 
Whew. You got the answer correct on the last try. 
How do you go from one number to the next?	increase bY 9
Correct! Congratulations! The pattern was increase bY 9. 
...
How do you go from one number to the next?	i don't know
Your input, 'i' was not accepted. 
Sorry, your answer was incorrect. The pattern was increment by 8
Would you like to play again? [Yes/No] 	no
Alright. Thank you for playing NumberSequence! 
...
class Main 
{
	...
	public void run() //this method runs the program. 
	{
        ...
        while (!userDone)
            //keep repeating until user doesn't want to play (until user is done)
        {
    		prompt();
                //prints prompt 
    		tries(getSequence());
                //gives in the number answer for the first try 
    		...
    		userDone = userPlayAgain();
    		if (!userDone)
    		    //if user wants to play again 
    		{
    		    System.out.println("");
    		        //make a buffer before restarting
    		}
        }
	}
	public boolean userPlayAgain()
	{
        ...
        boolean done = true;
            //to see if the user's done with the whole program; automatically initialize to true unless user wants to play again (reinitialized later)
        boolean validResponse = true;
            //automatically assume the user inputs a valid response (reinitialized later if not)
		do
		{
    		System.out.print("Would you like to play again? [Yes/No] \t");
    		String userPlayAgain = keyboard.next();
    		if (userPlayAgain.equalsIgnoreCase("yes"))
    		    //if user responded yes 
    		{
                System.out.println("\n");
                done = false;
    		}
    ...
    public void tries(int numAnswerIn)
        ...
        //this program runs nextNumProblem, and calls sequenceProblem if ready 
	{
        ...
        do
        {
            ...
            userTries++;
                //increment userTries every time this has to run
        } while (!userCorrect && userTries < 3);
            //if user didn't guess correctly and has more tries 
            //give another chance
        
        //user needs to continue on to the game; only 2 scenarios at this point: user succeeded or user failed. 
        if (userTries >= 3 && !userCorrect)
        ...
        else if (userTries <= 3 || userCorrect)
        ...
        //while generating and printing sequence, this method also returns number answer 
	{
		...
		int numAnswerReturn = outputSequence(startNum);
		System.out.println("___");
		return numAnswerReturn;
	}
	public int outputSequence(int startNumIn)
        ...
        //and returns the number answer 
	{
        ...
        {
            System.out.print((sequenceRule*count+startNumIn) + ", ");
                //prints out sequence using form y=mx+b 
        }
		int generateNumAnswer = sequenceRule*count+startNumIn;
